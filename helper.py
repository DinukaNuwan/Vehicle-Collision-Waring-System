...
def convert_to_pixel(box_yolo, img, crop_range):
    ...
    box = box_yolo
    imgcv = img
    [x_min, x_max] = crop_range[0]
    [y_min, y_max] = crop_range[1]
    h, w, _ = imgcv.shape

    # Calculate left, top, width, and height of the bounding box
    left = int((box.x - box.w / 2.) * (x_max - x_min) + x_min)
    top = int((box.y - box.h / 2.) * (y_max - y_min) + y_min)

    width = int(box.w * (x_max - x_min))
...
def convert_to_cv2bbox(bbox, img_dim=(1280, 720)):
    """Helper function for converting bbox to bbox_cv2
